...
- **ANI / pairwise-identity matrix** (`plotly-ani-heatmap.html`): full symmetric matrix, custom `colorscale` with explicit `zmin`/`zmax` (never let Plotly auto-range an identity scale — the 88–100 band is the whole message); per-cell value labels via a **`mode: "text"` scatter overlay** on the same category axes (heatmap `textfont.color` is single-valued — per-cell white/black contrast is impossible on the heatmap trace itself); `<b>`/`<i>` pseudo-HTML works in overlay text and tick labels (italic genus labels); `yaxis.autorange: "reversed"` puts row 1 on top like a matplotlib `imshow`; always symmetrize fastANI over query/reference directions BEFORE plotting and say so in the title.
- **Stacked / 100% composition bars (relative abundance)** (`plotly-relative-abundance.html`): one bar trace per taxon, `layout.barmode: "stack"`; for relative abundance either pre-normalize rows to 100% or set `yaxis.ticksuffix: "%"` with `range: [0, 100]`. Conventions: **lump low-abundance taxa into "Other"** (grey, last trace = top of stack; keep ≤ 12 named taxa visible); **stack order = trace order** (bottom-up), legend `traceorder: "reversed"` so the legend matches visual stacking; use a **qualitative palette** (Tableau/Okabe-Ito derived), never rainbow, and keep colors consistent across figures of the same study; order samples by grouping/condition, not alphabetically. Percent normalization lives in the data, not the axis.

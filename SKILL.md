...
| 15  | Maps — local site maps on raster tile basemaps (OSM/Esri/Carto, offline after build) OR regional occurrence/reference maps in vector style (NE 50m GeoJSON from R, degree graticule, shape-encoded points, place-name annotations)                            | **GeoMap**                       | `templates/geomap.html`           | §5k           | `engines/geomap.md`      |
...
- **GIS / geographic maps** → **never routed to the chart engines** (plotly/echarts/chartjs/cytoscape carry no projection, basemap, or geodetic semantics — two shipped experiments failed on exactly this). The only in-skill map engine is **GeoMap** (§5k: raster tile basemaps for local site maps; vector reference-map mode for regional occurrence figures, GeoJSON supplied by the GIS stack). Everything else — region choropleths, projected world maps, raster/satellite analysis figures, 3D terrain, interactive web maps — is delegated to the GIS stack: Python `geopandas`/`cartopy`/`geoplot`/`contextily`/`folium`/`pydeck`, R `sf`/`terra`/`tmap`/`ggplot2::geom_sf`/`leaflet`/`rayshader`; place-name → coordinates via the **geocoding skill**. chartz consumes their outputs (coordinates/GeoJSON) only.

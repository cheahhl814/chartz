...
- **GIS / geographic maps** → **never routed to the chart engines** (plotly/echarts/chartjs/cytoscape carry no projection, basemap, or geodetic semantics — two shipped experiments failed on exactly this). The only in-skill map engine is **GeoMap** (§5k: raster tile basemap + vector overlays; approved for site maps). Everything else — region choropleths, projected world maps, raster/satellite analysis figures, 3D terrain, interactive web maps — is delegated to the GIS stack: Python `geopandas`/`cartopy`/`geoplot`/`contextily`/`folium`/`pydeck`, R `sf`/`terra`/`tmap`/`ggplot2::geom_sf`/`leaflet`/`rayshader`; place-name → coordinates via the **geocoding skill**. chartz consumes their outputs (coordinates/GeoJSON) only.

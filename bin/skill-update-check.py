#!/usr/bin/env python3
"""chartz skill update check (v0.1.0).

Compares the deployed skill's git SHA + version to the upstream repo
(github.com/cheahhl814/chartz). No network calls beyond `git fetch`.
...
Usage: python3 bin/skill-update-check.py
"""
...
def find_skill_root(start: Path) -> Path | None:
    ...
    Also tries Path.cwd() — the user typically runs `python3 bin/skill-update-check.py` from
